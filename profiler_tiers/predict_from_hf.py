...
telecharger les poids (2026-09-06, corrige 2026-09-13 — voir CORRECTIF v4
ci-dessous), en combinant :
...
CORRECTIF v4 (2026-09-13) : la version 2026-09-06 de ce fichier calculait
TOUJOURS une taille hypothetique "si le modele etait en Q4_0", quel que soit
le fichier --file reellement demande (--file Modele-Q6_K.gguf donnait le
meme resultat que --file Modele-Q4_0.gguf — bug confirme en audit). Corrige
ici : le type REEL de chaque tenseur est lu depuis l'en-tete GGUF distant
(deja recupere, juste jamais agrege avant) et sert de base a l'affichage de
taille ET a un nouveau score de risque HTP (couverture MUL_MAT reelle par
octets pondere par frequence d'execution, + risque SOFT_MAX pour les MoE),
calibre sur deux mesures reelles independantes (Nanbeige4.2-3B, Marco-Nano,
2026-09-13) — voir capability_db.CAPABILITY_ENTRIES pour le detail sourc.
...
# ---------------------------------------------------------------------------
# CORRECTIF v4 (2026-09-13) — couverture HTP reelle par tenseur, pas une
# taille hypothetique. Source exacte : ggml_hexagon_supported_mul_mat()
# (ggml-hexagon.cpp:4567-4650) et ggml_hexagon_supported_softmax()
# (ggml-hexagon.cpp:4848-4911), fork self-build-jz commit 6be3d3dd, lues et
# verifiees directement le 2026-09-13. Voir aussi capability_db.CAPABILITY_
# ENTRIES (2 entrees ajoutees cette date) pour le detail des incidents source.
# ---------------------------------------------------------------------------
HTP_MUL_MAT_COVERAGE = {
    "F32": True, "F16": True,
    "Q4_0": True, "Q4_1": True, "Q8_0": True, "IQ4_NL": True, "MXFP4": True,
    "Q2_K": False, "Q3_K": False, "Q4_K": False, "Q5_K": False,
    "Q6_K": False, "Q8_K": False, "Q1_0": False, "Q2_0": False,
    "IQ1_S": False, "IQ2_XXS": False, "IQ3_XXS": False,
}

# Familles de tenseurs recurrentes (1x/couche, executees a CHAQUE token en
# decode) vs isolees (1x par forward pass, cout amorti) — classification
# heuristique par nom, PAS tracee depuis un vrai graphe d'execution.
...
def _classify_tensor_name(name):
    low = name.lower()
...
def compute_htp_mul_mat_risk(tlist):
    """Score de risque MUL_MAT->CPU, pondere par frequence d'execution (pas
    seulement par octets). Calibre sur Nanbeige4.2-3B (2026-09-13) : 6.8% des
    octets MUL_MAT rejetes (attn_q/output en Q6_K) -> ratio de temps CPU/HTP
    scheduler mesure = 50.8x. Corrige le bug 2026-09-06 (taille toujours
    hypothetique Q4_0, jamais le type reel du fichier demande)."""
    unsupported_recurrent, unsupported_isolated, unsupported_unknown = [], [], []
    total_bytes = unsupported_bytes = 0
    for name, _dims, ttype_code, nb in tlist:
        ...
        cls = _classify_tensor_name(name)
        (unsupported_recurrent if cls == "recurrent" else
         unsupported_isolated if cls == "isolated" else
         unsupported_unknown).append((name, type_name, nb))

    htp_covered_pct = 100.0 * (1.0 - (unsupported_bytes / total_bytes if total_bytes else 0.0))

    if unsupported_recurrent:
        risk, verdict = "VERY_HIGH", "REQUANTIFIER"
        rationale = (f"{len(unsupported_recurrent)} tenseur(s) recurrent(s) non "
                    f"supporte(s) (ex: {unsupported_recurrent[0][0]}, type "
                    f"{unsupported_recurrent[0][1]}) — motif identique a "
                    f"Nanbeige4.2-3B, ratio CPU/HTP mesure 50.8x.")
    elif unsupported_isolated:
        risk, verdict = "MODERATE", "SURVEILLER"
        rationale = (f"{len(unsupported_isolated)} tenseur(s) isole(s) non supporte(s) "
                    f"(ex: {unsupported_isolated[0][0]}) — cout amorti, probablement acceptable.")
    elif unsupported_unknown:
        risk, verdict = "UNKNOWN", "A VERIFIER MANUELLEMENT"
        rationale = f"{len(unsupported_unknown)} tenseur(s) non supporte(s) de famille non reconnue."
    else:
        risk, verdict = "LOW", "OK"
        rationale = "Couverture HTP MUL_MAT complete sur les types presents."

    return {"htp_covered_pct": htp_covered_pct, "risk": risk, "verdict": verdict,
           "rationale": rationale, "unsupported_recurrent": unsupported_recurrent,
           "unsupported_isolated": unsupported_isolated}


def check_softmax_moe_risk(n_experts):
    """SOFT_MAX du routeur MoE rejete si ne0 (=n_experts) > 32 et non multiple
    de 32 — verifie sur Marco-Nano (ne0=232, 232%32=8) le 2026-09-13."""
    if not n_experts or n_experts <= 1 or n_experts <= 32:
        return None
    aligned = (n_experts % 32) == 0
    return {"at_risk": not aligned, "n_experts": n_experts,
           "reason": (f"{n_experts} % 32 = {n_experts % 32} -> " +
                     ("aligne, devrait passer HTP" if aligned else
                      "NON aligne, SOFT_MAX du routeur rejete -> CPU a chaque "
                      "couche/token (motif Marco-Nano, +31.7% tg32 mesure en "
                      "corrigeant, non deploye sans validation numerique)"))}
...
def predict_no_download(repo_id, filename, budget_gb=9.0, safety_margin_gb=1.5):
    ...
    routing -> risque de crash -> risque HTP reel (v4). AUCUN octet de poids
    telecharge."""
    ...
    if n_experts and n_experts > 1:
        ...
        note += (" [RECALIBRATION EN ATTENTE 2026-09-13 : ce facteur precede "
                "la fusion MUL_MAT_ID_NX du fork self-build-jz (+43.5% mesure "
                "sur Marco-Nano le 2026-09-13) — probablement obsolete pour "
                "un build integrant cette fusion, non remesure a ce jour.]")
    else:
        factor, confidence, note = predict_dense_factor(a["l3"]["total_bytes_gb"])
    corrected_tps = l3_cold_tps * factor

    # CORRECTIF v4 : type REEL du fichier demande (pas une detection par nom
    # de fichier limitee a "q4_0"/"q8_0", et pas une taille toujours Q4_0
    # hypothetique — bug 2026-09-06 confirme en audit le 2026-09-13).
    htp_risk = compute_htp_mul_mat_risk(tlist)
    softmax_risk = check_softmax_moe_risk(n_experts)
    bytes_by_type = {}
    for _name, _dims, ttype_code, nb in tlist:
        tn = pm.V_TYPES.get(ttype_code, f"UNKNOWN_{ttype_code}")
        bytes_by_type[tn] = bytes_by_type.get(tn, 0) + nb
    total_gib_real = sum(bytes_by_type.values()) / 2**30
    dominant_type = max(bytes_by_type, key=bytes_by_type.get) if bytes_by_type else "?"
    risk = cdb.predict_crash_risk(dominant_type, "MUL_MAT", "HTP")

    total_gib_q4 = total_gib_real  # CORRECTIF v4 : taille reelle, plus l'estimation Q4_0 hypothetique
    ...
    return {
        ...
        "dominant_type": dominant_type, "bytes_by_type": bytes_by_type,
        "htp_risk": htp_risk, "softmax_risk": softmax_risk,
    }
...
def render_prediction(p):
    ...
    L.append(f"- taille REELLE du fichier demande : {p['size_q4_gib']:.2f} GiB "
             f"(type dominant : {p['dominant_type']}) — "
             f"{'TIENT' if p['fits_budget'] else 'NE TIENT PAS'} dans le budget")
    if len(p["bytes_by_type"]) > 1:
        detail = ", ".join(f"{t}={b/2**30:.3f} GiB" for t, b in
                           sorted(p["bytes_by_type"].items(), key=lambda kv: -kv[1]))
        L.append(f"  [MIXTE] {len(p['bytes_by_type'])} types de tenseurs presents : {detail}")
    L.append("")
    hr = p["htp_risk"]
    L.append(f"- Couverture HTP MUL_MAT reelle : {hr['htp_covered_pct']:.1f}%")
    L.append(f"- **Risque HTP : {hr['risk']} — Verdict : {hr['verdict']}**")
    L.append(f"  {hr['rationale']}")
    if hr["unsupported_recurrent"]:
        for name, ttype, nb in hr["unsupported_recurrent"][:5]:
            L.append(f"    - {name} ({ttype}, {nb/2**20:.1f} MiB)")
        if len(hr["unsupported_recurrent"]) > 5:
            L.append(f"    ... et {len(hr['unsupported_recurrent'])-5} de plus")
    sm = p.get("softmax_risk")
    if sm:
        L.append(f"- Risque SOFT_MAX routeur MoE : {'RISQUE' if sm['at_risk'] else 'OK'} — {sm['reason']}")
    L.append("")
