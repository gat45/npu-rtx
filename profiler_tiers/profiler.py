...
# CORRIGE v4 (2026-09-13) : Q4_1/IQ4_NL/MXFP4 sont bien HTP-natifs, confirme
# par lecture directe de ggml_hexagon_supported_mul_mat() (ggml-hexagon.cpp:
# 4567-4650, fork self-build-jz 6be3d3dd) — l'exclusion 2026-09-06 etait
# seulement pour coherence avec pm.BPW/pm.WBITS (profile_model.py, qui ne les
# liste pas et NE DOIT PAS etre modifie). Etendus ici localement (_WBITS_EXT)
# pour que _best_htp_native_leq() puisse au moins les proposer en repli HTP ;
# LIMITE ASSUMEE : le plan de quant par couche (quant_plan_safe, ORDER-based,
# lignes ~622/689/729/751 ci-dessous) continue de choisir uniquement parmi
# pm.ORDER (qui ne contient pas ces 3 types) — les proposer comme cible
# d'allocation directe demanderait d'etendre pm.ORDER/pm.BPW dans
# profile_model.py, explicitement hors de portee (fichier original intouche).
HTP_NATIVE_TYPES = {"F16", "Q8_0", "Q4_0", "Q4_1", "IQ4_NL", "MXFP4"}
_WBITS_EXT = dict(pm.WBITS)
_WBITS_EXT.setdefault("Q4_1", 4)
_WBITS_EXT.setdefault("IQ4_NL", 4)
_WBITS_EXT.setdefault("MXFP4", 4)
# _BPW_EXT : octets/poids (block de 32 elements), meme unite que pm.BPW.
# Q4_1 = 4 bits*32/8 + 2 scales fp16 (d,m) = 16+4 = 20 octets/bloc de 32.
# IQ4_NL = 4 bits*32/8 + 1 scale fp16 = 16+2 = 18 octets/bloc de 32.
# MXFP4 = 4 bits*32/8 + 1 scale e8m0 (1 octet) = 16+1 = 17 octets/bloc de 32.
# Non verifie en lisant le kernel HTP (contrairement a Q4_0/Q8_0 dans
# capability_db.py) — a confirmer avant de faire confiance a une taille
# GiB precise pour ces 3 types specifiquement.
_BPW_EXT = dict(pm.BPW)
_BPW_EXT.setdefault("Q4_1", 20 / 32)
_BPW_EXT.setdefault("IQ4_NL", 18 / 32)
_BPW_EXT.setdefault("MXFP4", 17 / 32)
...
def _best_htp_native_leq(fmt):
    ...
    order_by_bits = sorted(HTP_NATIVE_TYPES, key=lambda f: -_WBITS_EXT[f])
    for f in order_by_bits:
        if _WBITS_EXT[f] <= _WBITS_EXT.get(fmt, pm.WBITS.get(fmt, 0)):
            return f
    return min(HTP_NATIVE_TYPES, key=lambda f: _WBITS_EXT[f])
...
def quant_plan_safe(a, budget_gb, backend="HTP", safety_margin_gb=1.5,
    ...
    below_floor_notes = []
    total_now = sum(a["by_family"][f][1] * _BPW_EXT[alloc[f]] / 2.0 for f in alloc)
    if total_now > effective_budget:
        ...
        while total_now > effective_budget and guard < 60:
            ...
            if not moved:
                break
            total_now = sum(a["by_family"][f][1] * _BPW_EXT[alloc[f]] / 2.0
                            for f in alloc)
        if total_now > effective_budget:
            below_floor_notes.append(
    ...
    if backend == "HTP":
        for fam in HTP_OFFLOAD_FAMILIES:
            ...
            if fmt not in HTP_NATIVE_TYPES:
                ...
                gib_native = a["by_family"][fam][1] * _BPW_EXT[native] / 2.0
                gib_current = a["by_family"][fam][1] * _BPW_EXT[fmt] / 2.0
    ...
    if backend == "OpenCL-MoE" or moe_target_backend == "OpenCL":
        if "moe" in alloc and alloc["moe"] not in OPENCL_MOE_NATIVE_TYPES:
            gib_q40 = a["by_family"]["moe"][1] * _BPW_EXT["Q4_0"] / 2.0
            gib_current = a["by_family"]["moe"][1] * _BPW_EXT[alloc["moe"]] / 2.0
    ...
    total = sum(a["by_family"][f][1] * _BPW_EXT[alloc[f]] / 2.0
                for f in alloc)
    return {
        "alloc": alloc,
        "total_gib": total,
...
def render_safe_plan(plan, a):
    ...
    for fam, fmt in sorted(plan["alloc"].items(), key=lambda kv: -a["by_family"][kv[0]][1]):
        gib = a["by_family"][fam][1] * _BPW_EXT[fmt] / 2.0
...
def predict_layer_quant_impact(rows, a, budget_gb, live=None, safety_margin_gb=1.5):
    ...
    def layer_gib(layer, fmt_):
        ...
        return sum(row["elems"] * _BPW_EXT[fmt_] for row in by_layer[layer]) / 2**30
    ...
    while guard < 10000:
        ...
        for l in by_layer:
            ...
            if delta_gib <= 0 or total_gib + delta_gib > effective_budget:
                continue
            delta_bits = _WBITS_EXT[next_fmt] - _WBITS_EXT[fmt[l]]
    ...
    quality_proxy = (sum(_WBITS_EXT[fmt[l]] * layer_gib(l, fmt[l]) for l in by_layer) /
