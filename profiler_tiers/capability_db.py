...
CAPABILITY_ENTRIES = [
    ...
    {
        "quant": "Q6_K", "op": "MUL_MAT", "backend": "HTP",
        "shape_constraint": "n'importe quelle geometrie — src0->type == GGML_TYPE_Q6_K "
        "n'est simplement pas dans le switch de ggml_hexagon_supported_mul_mat() "
        "(cases geres : Q4_0/Q4_1/Q8_0/IQ4_NL/MXFP4/F16/F32)",
        "status": "UNSUPPORTED", "source": "verified_in_code", "blast_radius": "none",
        # none : pas de crash, repli CPU silencieux decide par le SCHEDULER
        # (ggml_backend_sched) AVANT tout dispatch HTP — different du
        # HTP_STATUS_NO_SUPPORT au niveau firmware (entry.c) qui, lui,
        # avorte le batch. Deux mecanismes distincts, a ne pas confondre.
        "note": ("Nanbeige4.2-3B-Q4_0.gguf (bartowski) est nomme d'apres son "
                "format dominant (Q4_0) mais contient attn_q.weight et "
                "output.weight en Q6_K (choix qualite du quantizer tiers, "
                "quantification mixte). Consequence mesuree : ces 347/5099 "
                "MUL_MAT tombent en CPU, chacun a chaque couche/token pour "
                "attn_q -> ratio de temps scheduler CPU/HTP mesure = 50.8x "
                "(5,672,581 us CPU vs 111,735 us HTP, meme run). Requantifier "
                "attn_q/output en Q4_0 (source Q8_0 -> llama-quantize "
                "--tensor-type attn_q=q4_0 --tensor-type output=q4_0) donne "
                "100% MUL_MAT HTP et pp64 55.05->399.34, tg32 4.95->10.56 "
                "(meme device, meme session)."),
        "ref": "ggml-hexagon.cpp:4567-4650 (ggml_hexagon_supported_mul_mat), "
              "session 2026-09-13 (Nanbeige4.2-3B requant A/B)",
        "date": "2026-09-13",
    },
    {
        "quant": "n/a (F32, contrainte de forme pas de type)", "op": "SOFT_MAX",
        "backend": "HTP",
        "shape_constraint": "ne[0] (taille de ligne, ex. n_experts pour un routeur "
        "MoE) > 32 ET non multiple de 32 — condition exacte : "
        "ne0 > 32 && (ne0 & 31) != 0",
        "status": "UNSUPPORTED", "source": "verified_in_code", "blast_radius": "none",
        "note": ("Marco-Nano MoE : ffn_moe_logits presente ne[0]=232 a SOFT_MAX "
                "(232 % 32 = 8, non aligne) -> rejete par "
                "ggml_hexagon_supported_softmax(), repli CPU a CHAQUE couche/"
                "token (1015 frontieres CPU<->HTP par run mesure). Le "
                "commentaire du code avertit explicitement d'un probleme de "
                "tail-handling HVX reel pour cette taille, PAS seulement une "
                "garde conservatrice non testee — la correction numerique du "
                "kernel n'a PAS ete verifiee element-par-element a ce jour. "
                "Test de debit (bypass gate GGML_HEXAGON_FORCE_SOFTMAX_HTP=1, "
                "sans validation numerique) : tg32 18.73->24.67 (+31.7%), "
                "n=5/groupe, separation statistique complete (Mann-Whitney "
                "U=25, p=0.00794). NE PAS deployer le bypass sans validation "
                "numerique prealable."),
        "ref": "ggml-hexagon.cpp:4848-4911 (ggml_hexagon_supported_softmax), "
              "session 2026-09-13 (Marco-Nano SOFT_MAX A/B causal)",
        "date": "2026-09-13",
    },
]

# ---------------------------------------------------------------------------
# NOTE 2026-09-13 (recalibration a faire) : ROUTING_CALIBRATION dans
# predict_from_hf.py (points Marco-Nano/Qwen3-0.9B/Huihui/EuroMoE) a ete
# mesuree le 2026-09-06 sur un build ggml-hexagon ANTERIEUR a la fusion
# MUL_MAT_ID_NX du fork self-build-jz (commit 6be3d3dd, testee cette session
# le 2026-09-13 : +43.5% sur Marco-Nano en un seul A/B controle, cf.
# fused_op::try_fuse_mul_mat_id_nx). Le facteur x0.47 (232E/top8 vs L3) est
# donc tres probablement OBSOLETE pour tout build integrant cette fusion —
# a remesurer avant de faire confiance a predict_from_hf.py sur un modele
# MoE avec ce nombre d'experts. Ne PAS corriger la constante sans nouvelle
# mesure (pas de recalibration a l'aveugle).
# ---------------------------------------------------------------------------
